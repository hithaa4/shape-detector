...
export class ShapeDetector {
    ...
    detectShapes(imageData) {
        ...
        for (let i = 0; i < gray.length; i++)
            binary[i] = gray[i] < threshold ? 1 : 0; // foreground = 1
    ...
    _morphOpen(bin, w, h, iterations = 1) {
        let tmp = bin.slice();
    ...
    _traceContour(mask, mw, mh, offsetX = 0, offsetY = 0) {
        // find starting pixel (first border pixel)
        const idxOf = (x, y) => y * mw + x;
        let sx = -1, sy = -1;
        for (let y = 0; y < mh; y++) {
            for (let x = 0; x < mw; x++) {
    ...
    _rdp(points, eps) {
        ...
        const recurse = (pts, start, end, out) => {
