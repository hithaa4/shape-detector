...
    const minSize = Math.max(50, Math.floor((width * height) * 0.00001)); // tiny area threshold
...
      const epsilon = Math.max(0.2, 0.002 * Math.sqrt(area));
...
      // 8) Classify shape (Refactored to be sequential and clear)
      const avgAngleDeg = avgAngle * 180 / Math.PI;
      const ar = bbox.w / bbox.h;
      const arRatio = ar < 1 ? 1 / ar : ar; // Aspect Ratio always >= 1
      const sideCount = vcount;

      console.log(`Shape debug (refined) → sides: ${sideCount}, avgAngle: ${avgAngleDeg.toFixed(1)}, arRatio: ${arRatio.toFixed(2)}, circularity: ${circularity.toFixed(2)}`);

      if (sideCount === 3) {
        type = 'triangle';
        // Confidence based on how close the average angle is to 60° (equilateral)
        confidence = 0.85 + Math.max(0, 1 - Math.abs(avgAngleDeg - 60) / 40) * 0.15;
      } else if (sideCount === 4) {
...
        confidence = 0.9;
      } else if (sideCount > 5) {
        // For high vertex count, check circularity for a circle
        if (circularity > 0.75) {
          type = 'circle';
...
          confidence = 0.4;
        }
      } else { // sideCount < 3 (noise or too small)
        if (circularity > 0.85) { // Very circular despite low vcount
          type = 'circle';
          confidence = 0.8;
        } else {
          type = 'unknown';
...
      console.log(`Final Classification: ${type}, Confidence: ${confidence.toFixed(2)}`);
    
      // Push the final, correct result
      results.push({
        type,
        bbox,
        area,
